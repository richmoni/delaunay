...
def dt(lhs: list, rhs: list) -> None:
    ...
    plt.plot([x[0] for x in midpoints], [y[1] for y in midpoints], 'k+-')
    for i, vertex in enumerate(vertices):
        plt.text(vertex[0]-0.03, vertex[1]+0.03,
                 i, ha='right')  # label vertices
    for i, simplex in enumerate(tri.simplices):
        p = np.asarray(vertices)[simplex].mean(axis=0)
        plt.text(p[0], p[1], '#%d' % i, ha='center')  # label simplices
    plt.show()
